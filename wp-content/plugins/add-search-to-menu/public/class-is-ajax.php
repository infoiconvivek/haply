...
class IS_Ajax
{
	...
	function ajax_load_posts()
	{
		...
		$defaults = array(
			...
			'show_author'                => 0,
			'show_date'                  => 0,
			'nothing_found_text'         => __('Nothing found 4', 'add-search-to-menu'),
			'show_more_result'           => 0,
			'show_more_func'             => 0,
				...
				<span class="is-meta-tag">
					...
					<span class="is-tags-links">
						<?php foreach ($terms as $key => $term) {
							if ($key) {
								echo ', ';
							} ?><a href="<?php echo get_term_link($term->term_id, $post->post_type . '_tag'); ?> " rel="tag"><?php esc_html_e($term->name); ?></a><?php } ?>
					</span>
				</span>
			<?php }
		}
				...
				<span class="is-meta-category">
					...
					<span class="is-cat-links">
						<?php foreach ($terms as $key => $term) {
							if ($key) {
								echo ', ';
							} ?><a href="<?php echo get_term_link($term->term_id, $tax_name); ?> " rel="tag"><?php esc_html_e($term->name); ?></a><?php } ?>
					</span>
				</span>
			<?php }
		}
